...
public class EffectUIInspector {
    /// <summary>
    /// 特效Shader前缀
    /// </summary>
    ...
    //        EffectImage EffectImage = Undo.AddComponent<EffectImage>(obj);
    //        ImageProperty.PasteProperty(EffectImage, property);
    //        property = null;
    ...
    //        AdvancedRawImage EffectImage = Undo.AddComponent<AdvancedRawImage>(obj);
    //        RawImageProperty.PasteProperty(EffectImage, property);
    //        property = null;
    //        EditorUtility.SetDirty(obj);
    //    }
    //}
    ...
    //    if (cmd.context.GetType() == typeof(EffectImage)) {
    //        EffectImage EffectImage = cmd.context as EffectImage;
    //        GameObject obj = EffectImage.gameObject;
    //        ImageProperty property = ImageProperty.CopyProperty(EffectImage);
    //        Undo.DestroyObjectImmediate(EffectImage);
    ...
    //        AdvancedRawImage EffectImage = cmd.context as AdvancedRawImage;
    //        GameObject obj = EffectImage.gameObject;
    //        RawImageProperty property = RawImageProperty.CopyProperty(EffectImage);
    //        Undo.DestroyObjectImmediate(EffectImage);
    ...
    private EffectImage _effectImage;
    private List<EffectUIPlayer> _effectsPlayers;
    ...
    public EffectUIInspector(Graphic target, Editor editor) {
        _target = target;
        _editor = editor;
        if (_target is EffectImage) {
            _effectImage = _target as EffectImage;
            _effectsPlayers = _effectImage.EffectsPlayers;
            _isUnscaledDeltaTime = _effectImage.IsUnscaledDeltaTime;
        }
    ...
    public void OnInspectorGUI() {
        ...
        EditorGUILayout.LabelField("是否无视TimeScale?", GUILayout.Width(305));

        EditorGUI.BeginChangeCheck();
        _isUnscaledDeltaTime = EditorGUILayout.Toggle(_isUnscaledDeltaTime);
        // 如果有更改，应用更改并标记对象为已修改  
        if (EditorGUI.EndChangeCheck()) {
            if (_effectImage != null) {
                Undo.RecordObject(_effectImage, "Toggle Time Scale");
                _effectImage.IsUnscaledDeltaTime = _isUnscaledDeltaTime;
                EditorUtility.SetDirty(_effectImage);
            }
        }
    ...
    private void OnAnimationGUI() {
        ...
        for (int i = 0; i < _effectsPlayers.Count; i++) {
            EffectUIPlayer effectsPlayer = _effectsPlayers[i];
                ...
                if (_effectsPlayers.Exists((e) => { return e.PropertyName == property.name; })) {
                    ...
                    gm.AddItem(new GUIContent(property.displayName), false, () => {
                        ...
                        if (property.type == MaterialProperty.PropType.Float)
                            ...
                            valueType = AnimationValueType.Color;
                        else if (property.type == MaterialProperty.PropType.Vector)
                            valueType = AnimationValueType.Vector4;
                        _effectsPlayers.Add(new EffectUIPlayer(_target, property.name, property.displayName, valueType));
                        EndChange();
                    });
                }
            }
